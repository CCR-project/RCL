...
Goal wcompatible7 (_simg) bindC.
Proof.
  econs.
  { ii. eapply bindR_mon; eauto. }
  i. inv PR. csc.
  revert k_src k_tgt SIMK. induction SIM using _simg_ind2; i.
  { irw. hexploit SIMK; eauto. i.
    eapply simg_mon; eauto. eapply simg_flag.
     { eapply simg_mon; eauto. i. gbase. auto. }
    { ss. }
    { ss. }
  }
  { rewrite ! bind_bind. econs; eauto. ii.
...
| simg_safe_ret
    r_src r_tgt
    (SIM: condOn -> RR r_src r_tgt)
  :
    _simg_safe condOn simg RR f_src f_tgt (Ret r_src) (Ret r_tgt)
| simg_safe_syscall
    ktr_src0 ktr_tgt0 fn varg rvs
    (SIM: forall x_src x_tgt (EQ: x_src = x_tgt), simg _ _ RR true true (ktr_src0 x_src) (ktr_tgt0 x_tgt))
  :
    _simg_safe condOn simg RR f_src f_tgt (trigger (Syscall fn varg rvs) >>= ktr_src0) (trigger (Syscall fn varg rvs) >>= ktr_tgt0)

| simg_safe_tauL
    itr_src0 itr_tgt0
    (TAUL: True)
    (SIM: condOn -> simg _ _ RR true f_tgt itr_src0 itr_tgt0)
  :
    _simg_safe condOn simg RR f_src f_tgt (tau;; itr_src0) (itr_tgt0)
| simg_safe_tauR
    itr_src0 itr_tgt0
    (TAUR: True)
    (SIM: condOn -> simg _ _ RR f_src true itr_src0 itr_tgt0)
  :
    _simg_safe condOn simg RR f_src f_tgt (itr_src0) (tau;; itr_tgt0)

| simg_safe_chooseR
    X itr_src0 ktr_tgt0
    (CHOOSER: True)
    (SIM: condOn -> forall x, simg _ _ RR f_src true itr_src0 (ktr_tgt0 x))
  :
    _simg_safe condOn simg RR f_src f_tgt (itr_src0) (trigger (Choose X) >>= ktr_tgt0)

| simg_safe_takeL
    X ktr_src0 itr_tgt0
    (TAKEL: True)
    (SIM: condOn -> forall x, simg _ _ RR true f_tgt (ktr_src0 x) itr_tgt0)
  :
    _simg_safe condOn simg RR f_src f_tgt (trigger (Take X) >>= ktr_src0) (itr_tgt0)
.

Notation "p -7> q" :=
  (fun x0 x1 x2 x3 x4 x5 x6 => forall (PR: p x0 x1 x2 x3 x4 x5 x6 : Prop), q x0 x1 x2 x3 x4 x5 x6 : Prop)
  (at level 50, no associativity).

Definition _simg_safe'
  (simg: forall R0 R1 (RR: R0 -> R1 -> Prop), bool -> bool -> (itree eventE R0) -> (itree eventE R1) -> Prop):
  (forall R0 R1 (RR: R0 -> R1 -> Prop), bool -> bool -> (itree eventE R0) -> (itree eventE R1) -> Prop) :=
  @_simg_safe false simg -7> @_simg_safe true simg
.

Lemma simg_safe_spec:
  _simg_safe true <8= gupaco7 _simg (cpn7 _simg).
Proof.
...
(* TODO: See above proof for bind. (wcompatible7) respectful makes the proof easier than directly proving compatible. *)
(* Maybe apply that idea here. *)

Lemma simg_safe_spec2:
  (* forall x0 x1 x2 x3 x4 x5 x6, forall (PR: @simg x0 x1 x2 x3 x4 x5 x6), (@_simg_safe' simg) x0 x1 x2 x3 x4 x5 x6. *)
  simg <7= (@_simg_safe' simg).
Proof.
  i. punfold PR. induction PR using _simg_ind2.
  - econs; eauto.
  - econs; eauto. i. exploit SIM; try apply EQ; et. i; des. pclearbot. ss.
  - econs; eauto.
  - econs; eauto.
  - ii. inv PR; try (by irw in H0; csc).
    + clear SIM0. des.
      (** minki: commutativity of simg/simg_safe is essence? **)
      econs; eauto. i. pfold. econs; eauto. admit "".
    + des. econs; eauto. i. pfold.
    clarify. des. econs; eauto.
  - econs; eauto.
  - econs; eauto.
  - econs; eauto.
  - econs; eauto.
  - econs; eauto.
  eapply simg_indC_spec. inv PR.
  { econs; eauto. }
  { econs; eauto. }
  { econs; eauto. }
  { econs; eauto. }
...
Minki: compatible: clo f <= f clo
Above case: f clo <= clo f

YJ: f clo f <= clo f?
Minki: f clo (gfp) <= clo f (gfp)

simg (_simg_safe true simg x2 f_src f_tgt (` x : _ <- trigger (Choose X);; ktr_src0 x) (tau;; itr_tgt1))

Minki: logic programming? power를 잃지 않는
sequence logic? sequent calculus?

C11: prefix * beh >= beh * prefix
{ P } c1; c2 { Q } = ∃ R. { P } c1 { R } ∧ { R } c2 { Q }


core logic.
memory 처럼 Fair Event 마다 요구하는 PCM 이 있음
blocking algorithm 증명 --> 저기에 PCM 잘 버무리면 되고 재료는 저걸로 충분하다
TADA는 linear -> affine logic. logic semantics/model 이 많이 바뀜. assertion 추가되고
LiLi는 말할 필요도 없이... base logic이라고 되어있는거는 definite action... 이상한거 많고
***)
