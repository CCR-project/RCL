...
Section ADQ.
  ...
  Proof.
    ...
    - unfold HoareCall, ASSUME, ASSERT, mput, mget. steps.
      force_l. exists None. steps. force_l. exists (fr, ε, ε). steps.
      force_l.
      { rewrite ! URA.unit_id. refl. }
      steps. force_l. exists args. steps.
      force_l; ss.
      ...
      rr in _ASSUME0. uipropall. subst. destruct w1.
      deflag. gbase. eapply CIH; et.
      rewrite ! URA.unit_id in _ASSUME. rewrite ! URA.unit_id. ss.
      Unshelve.
      all: try (exact Ord.O).
  ...
  Proof.
    ...
    { des_ifs. }
    assert (exists b, x1 = Any.pair b t).
    { destruct x; ss.
  ...
  Proof.
    ...
    { instantiate (1:=fun _ '(st_src, st_tgt) =>
      ...
      { rr in _ASSUME0. uipropall. des. rr in _ASSUME0. uipropall.
        ...
        force_r. exists true.
        force_r. exists m. force_r. eexists x0.
        force_r. force_r; auto.
        force_r. eexists x. force_r; et.
        ...
        { steps. guclo lbindC_spec. econs.
          ...
          force_l. eexists (c0, c1, c). force_l. steps. force_l; et. force_l. force_l. force_l. eexists.
          force_l; et.
        ...
        { steps. guclo lbindC_spec. econs.
          ...
          i. ss. des; clarify. destruct vret_tgt as [fr0 vret].
          steps. force_l. eexists (c0, c1, c). steps.
          force_l; et. steps. force_l. eexists _. steps.
          force_l; et.
          steps. econs; ss. esplits; et.
        }
      }
      { rr in _ASSUME0. uipropall. subst.
        ...
        force_r. exists tt. force_r. exists x0.
        ...
        i. ss. des; clarify. destruct vret_tgt as [fr0 vret].
        steps. force_l. eexists (c0, c1, c).
        steps. force_l; et. steps.
        force_l; et. eexists. steps. force_l; et.
        steps. econs; ss. esplits; et.
      }
    }
